...
def report(x, step_len, f_val, change_in_f, itr):
    # print('itr: %d location: %f, previous location: %f current value: %f , previous value: %f ' % itr, x, previous_x,
    #       f_val, previous_f_val)
    print("itr: %d, location: %s , location, step_len: %f, function value: %f , change in f: %f " %
          (itr, str(x), step_len, f_val, change_in_f))
...
def is_converged(change_in_f, step_len, obj_tol, param_tol):
    if change_in_f < obj_tol:
        print('obj tolerance')
        return True
    elif step_len < param_tol:
        print('param tolerance')
        return True
    else:
        return False
...
def gradient_descent(f, x0, step_size, obj_tol, param_tol, max_iter):
    ...
    for itr in range(max_iter):
        ...
        report(x, step_len, current_f_value, change_in_f, itr)
        previous_x = x
        x = x - step_size * grad
...
def find_step_len_wolf_condition(x_k, p_k, c_1, alpha_0, f, f_grad, back_track_factor):
    is_objective = False
    new_alpha = alpha_0
    itr_cnt = 0
    while is_objective is False:
        ...
        itr_cnt += 1
        if itr_cnt > 1000:
            print('find_step_len failed in itr: ', itr_cnt)
            return alpha_0
    print('find_step_len success in itr: ', itr_cnt)
    return new_alpha
...
def line_search(f, x0, step_size, obj_tol, param_tol, max_iter, dir_selection_method, init_step_len=1.0,
    ...
    for itr in range(max_iter):
        ...
        if itr == 0:
            ...
            new_alpha = find_step_len_wolf_condition(previous_pk, previous_pk, c_1, init_step_len, f, grad, back_track_factor)
        # defining the direction
        #Updating x_k+1

        if dir_selection_method == 'gd':
            p_k = - step_size * grad
        elif dir_selection_method in ['nt', 'bfgs']:
